...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        """
        a = torch.sqrt(torch.pow(x, 2).mean(-1, keepdim=True) + self.eps)
        b = torch.div(x, a)
        return b
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        normalized_input = self.attention_norm(x)
        attention_output = self.attention(normalized_input)
        attention_residual = x + attention_output
        normalized_attention = self.ffn_norm(attention_residual)
        ffn_output = self.feed_forward(normalized_attention)
        output = attention_residual + ffn_output
